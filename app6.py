    ...
    if "replyToken" in events[0]:
        ...
        if events[0]["type"] == "message":
            if events[0]["message"]["type"] == "text":
                ...
                if text == "這隻貓叫作什麼名字?":
                    payload["messages"] = [openCamera()]          # Line-sever 接受的訊息格式
                    replyMessage(payload)                         # 呼叫回傳訊息
                    print(type(payload))
                    print('payload function1------->'+str(payload))
        ...
        if events[0]["message"]["type"] == "image":
            ...
            # whatscat(local_save, replyToken, events[0]["message"]["id"])     # 呼叫功能一: 這隻貓叫作什麼名字
            payload["messages"] = [flexmessage()]
...
def saveimg(message_id):
    SendImage = line_bot_api.get_message_content(message_id)     # message_id 用戶傳訊息的訊息ID
...
def flexmessage():
    flex_contents = "./flexmessage_template/跩哥Malfoy_flexmessage.json"
